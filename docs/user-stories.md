...
* **Критерии приёмки (AC):**
  ...
  - **AC 1.2:** На главном экране отображается краткая справка или приветственное окно с инструкцией для новых пользователей.
...
* **Роль:** Исследователь родословной / Сотрудник архива
* **История:** Я как исследователь хочу выбрать папку на локальном компьютере, чтобы указать системе, где находятся архивные документы, которые нужно обработать.
* **Критерии приёмки (AC):**
...
* **Definition of Done:**
  ...
  - [ ] Выбранные параметры сохраняются в сессию (в памяти) или в локальное хранилище браузера
...
* **Definition of Done:**
  ...
  - [ ] Интеграционный тест: обработка папки с повреждежными файлами → проверка error log
...
* **Definition of Done:**
  - [ ] API endpoint `POST /api/export` разработан и поддерживает все форматы
...
* **Критерии приёмки (AC):**
  - **AC 9.1:** Приложение содержит встроенный механизм проверки обновлений (например, кнопка «Проверить обновления» в меню).
